...
export function toMigrationSlug(input: string) {
  return input
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
...
export function resolveDirectory(cwd: string, input?: string) {
...
export function listSqlFiles(directory: string) {
  if (!fs.existsSync(directory)) {
    return [];
  }

  return fs.readdirSync(directory).filter((file: string) => file.endsWith(".sql"));
}

export function groupMigrationFiles(files: string[]) {
  ...
  for (const file of files) {
    ...
    group[direction as "up" | "down"] = file;
...
function loadTemplate(templatesDirectory: string, fileName: string) {
  return fs.readFileSync(path.join(templatesDirectory, fileName), "utf8");
}

function readOption(options: Record<string, string | boolean>, key: string) {
...
export function createMigration(input: {
  ...
  if (!input.name) {
    throw new CliError(
      "Please provide a migration name. Example: rn-sqlite-migrations create split_full_name",
    );
  }
  ...
  if (!slug) {
    throw new CliError(
      "The migration name produced an empty slug. Use letters or numbers in the name.",
    );
  }
  ...
  const upTemplate = loadTemplate(input.templatesDirectory, "migration.up.sql").replace(
    /__MIGRATION_NAME__/g,
    baseName,
  );
  const downTemplate = loadTemplate(input.templatesDirectory, "migration.down.sql").replace(
    /__MIGRATION_NAME__/g,
    baseName,
  );
...
export function validateMigrations(input: {
  cwd: string;
  io: CliIo;
  options: Record<string, string | boolean>;
}) {
...
export function generateManifest(input: {
  ...
  const output = path.resolve(
    input.cwd,
    readOption(input.options, "out") || path.join(directory, "manifest.generated.json"),
  );
  const files = listSqlFiles(directory);
  const groups = groupMigrationFiles(files);
  const manifest: Record<string, { down: string | null; up: string | null }> = {};

  for (const [baseName, group] of [...groups.entries()].sort((left, right) =>
...
export function runCli(options: CliRuntimeOptions) {
  const parsed = parseArgs(options.argv);
  const cwd = options.cwd ?? process.cwd();
  const templatesDirectory =
    options.templatesDirectory ?? path.resolve(cwd, "templates");
  ...
  try {
    switch (parsed.command) {
      ...
      default:
        throw new CliError(
          `Unknown command "${parsed.command}". Run "rn-sqlite-migrations help" for usage.`,
        );
    }
  } catch (error) {
